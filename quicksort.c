...
void quicksort(int array[], int left, int right,int *change);
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i<n; i++) {// N回計測
		...
		quicksort(array, 0, size - 1,&change);
...
void quicksort(int array[], int left, int right,int *change) {
	int l = left;
	int r = right;
	int p = median(array[l], array[((l + r) / 2)], array[r]);
	int tmp;
	...
	if (left < l - 1) quicksort(array, left, l - 1,change);
	if (r + 1 < right) quicksort(array, r + 1, right,change);
}
